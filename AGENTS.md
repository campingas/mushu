...
- **`install.sh` replaces binaries with `install`, never `cp`, and never restarts a service.** `cp` over a running executable fails with `Text file busy` on Linux, so an update would break on exactly the hosts that are already serving. `install` replaces the inode instead, which is also why the live process keeps running the previous version until the owner chooses to restart it (verified on uutils coreutils 0.8.0; GNU coreutils is expected to behave the same but is untested).
